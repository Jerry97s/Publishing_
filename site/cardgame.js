...
(function () {
  ...
  function bindEvents() {
    ...
    root.querySelector("#card-win-close").addEventListener("click", () => {
      el.winModal.classList.remove("is-open");
      newGame();
    });
    root.querySelector("#card-board").addEventListener("click", handleClick);
    root.querySelector("#card-board").addEventListener("dblclick", handleDblClick);
  }
  ...
  window.initCardGame = function () {
    ...
    el = {
      ...
      foundations: [0, 1, 2, 3].map((i) => root.querySelector("#card-found-" + i)),
      tableau: [0, 1, 2, 3, 4, 5, 6].map((i) => root.querySelector("#card-column-" + i)),
      moves: root.querySelector("#card-stat-moves"),
      score: root.querySelector("#card-stat-score"),
      time: root.querySelector("#card-stat-time"),
      winModal: root.querySelector("#card-win-modal"),
    };
    bindEvents();
    newGame();
  };
